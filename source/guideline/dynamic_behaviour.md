...
The first method is presented in [@fig:dynamic_behaviour_get_operation], where the service consumer uses directly the Get operation of the service provider. By utilising the available operation parameters such as the geometry and the time-period, only specific S-125 datasets matching the specified criteria and/or have updates on the specified time-period interval, will be retrieved, packaged into an S-100 Exchange Set, and returned as a response. 
